<template>
    <div  class=" full_layer_row" @mouseover="getDynamicIndexOnHover">
         ...
         <div  class="full_data"  >
             <div  draggable="true" @dragstart="dragStartInner(index, $event)" @dragover.prevent @dragenter="dragEnter" @dragleave="dragLeave" @dragend="dragEndInner" @drop="dragFinishInner(index, $event)" v-for="(divData, index) in dynamicHtml" :key="index" :class="{'col-sm-3':divData.className =='col-sm-3' , 'cell':true, 'eleven':divData.type =='eleven', 'twelve' :divData.type =='twelve', 'thirteen' :divData.type =='thirteen', 'fourteen' :divData.type =='fourteen' }" :id="divData.id"  :style="divData.style">
                <!-- buttons on hovers -->
                 <!-----------------Conditional button--------------->
                ...
                <div class="Copy_component"  v-if="divData.values[0].text || divData.values[1].image || divData.values[2].video ">
