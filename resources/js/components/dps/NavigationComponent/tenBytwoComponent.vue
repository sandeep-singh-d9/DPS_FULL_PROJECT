<template>
   <div>
        <div class=" full_layer_row" @mouseover="getDynamicIndexOnHover">
            ...
            <div  class="full_data">
                <div  draggable="true" @dragstart="dragStartInner(index, $event)" @dragover.prevent @dragenter="dragEnter" @dragleave="dragLeave" @dragend="dragEndInner" @drop="dragFinishInner(index, $event)" :class="{'col-sm-10': divData.className =='col-sm-10' ,
                 'col-sm-2': divData.className =='col-sm-2' , 'twentyone' : divData.type == 'twentyone', 'twentytwo' : divData.type == 'twentytwo', 'cell': true}" v-for="(divData, index) in dynamicHtml" :key="index" :id="divData.id"  :style="divData.style">
                    <!-- buttons on hovers -->
                     <!-----------------Conditional button--------------->
